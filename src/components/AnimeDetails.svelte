...
Dataset Description ${dataset.Data_Description}`;
  ...
  const papersList = pubData.data.map((paper, index) => `
    Paper ${index + 1}:
      Title: ${paper.title}
      Year: ${paper.year}
...
`;
  ...
  let embeddedPointByID: Map<number, EmbeddedPointWithIndex> = new Map(
    embedding.map((p) => [+p.metadata.id, p])
  );
...
<div class="root"> 
  <div class="details"> 
    <div class="info"> 
      ...
      <h2> 
        <a href="https://aimed.uab.edu/apex/r/tkg/ckg/author-details?p10_aid={datum.metadata.id}" target="_blank" onClick={() => navigateTo(datum.metadata.id)}> 
      ...
      <p>Institution:</p>  {datum.metadata.Affiliation}
      <p>Career Begin Year:</p>  {datum.metadata.BeginYear}
      <p>Number of Papers: </p> {datum.metadata.PaperNum} <br><br>
      
      <button class="rec_button" onclick="window.open('https://aimed.uab.edu/apex/r/tkg/ckg/author-details?p10_aid=' + encodeURIComponent('{datum.metadata.id}'));">
        Details
      </button>
    ...
    <br><br>
    <button class="rec_button" onclick="window.open('https://www.google.com/search?q=' + encodeURIComponent('{datum.metadata.FullName} {datum.metadata.Data_Description}'));">
      Google
    </button>
    <br>
    <h2>Potential Future Users ({embeddingNeighbors[datum.index].length})</h2>
{/if}
      ...
      <ul> 
        {#each embeddingNeighbors[datum.index]
          ...
          .sort((a, b) => embeddedPointByID.get(b)?.metadata?.BeginYear - embeddedPointByID.get(a)?.metadata?.BeginYear) 
          as neighborId, i (neighborId)} 
          {#if embeddedPointByID.get(neighborId)}
            <li> 
              ...
              {#if datum.metadata.IsAuthor}
              <button class="small_button" on:click={() => handleWhyRecommendClick(datum.metadata.id, neighborId)} disabled={fetchingExplanations[`${datum.metadata.id}_${neighborId}`]}>
              ...
              {#if datum.metadata.IsAuthor == false}
              <button class="small_button" on:click={() => handleWhyRecommendClickDataset(datum.metadata.id, neighborId)} disabled={fetchingExplanations[`${datum.metadata.id}_${neighborId}`]}>
                Why Recommend? (ChatGPT)
              </button>
              {/if}
              <button class="rec_button" onclick="window.open('https://aimed.uab.edu/apex/r/tkg/ckg/author-details?p10_aid=' + encodeURIComponent('{embeddedPointByID.get(neighborId).metadata.id}'));">
                Details
              </button>



              <button class="small_button" onclick="window.open('https://www.google.com/search?q=' + encodeURIComponent('{embeddedPointByID.get(neighborId).metadata.FullName} {embeddedPointByID.get(neighborId).metadata.Affiliation}'));">
                Google
              </button>
